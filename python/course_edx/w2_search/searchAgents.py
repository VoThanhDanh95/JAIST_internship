...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # print('dxy',dx,dy)
            nextx, nexty = int(x+dx), int(y+dy)
            # print('nextdx',nextx,nexty)
            hitsWalls = self.walls[nextx][nexty]
            # print('hitsWalls', hitsWalls)
            if not hitsWalls:
                if (nextx, nexty) in self.corners and (nextx, nexty) not in visited_corner:
                    explored_corners = visited_corner + [(nextx, nexty)]
                    successors.append((((nextx, nexty), explored_corners),action,1))
                else:
                    successors.append((((nextx, nexty), visited_corner),action,1))
        # print successors
        self._expanded += 1 # DO NOT CHANGE
        return successors
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        heuristic += util.manhattanDistance(node, corner)
    return heuristic
    ...
    #     manhattanDistance += min([util.manhattanDistance()])
    # "*** YOUR CODE HERE ***"
    # # return 0
    # return manhattanDistance # Default to trivial solution
